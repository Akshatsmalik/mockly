...
function Navbar({ action }) {
  ...
  const condition = questionc>=numq
  const {results}=useContext(Data)
  ...
  return (
        <div className={`w-full mx-100vh border-2 px-[40px] border-gray-300 bg-gray-200 rounded-3xl`}>
        <header className="flex justify-between items-center bg-gr px-4 py-5">
      ...
      <div className="w-[160px] h-[44px] flex items-center justify-end">
        <div className="flex items-center gap-6">
            <button
              ...
              className={`font-medium py-[10px] px-[32px] rounded-[10px] transition-all duration-200 shadow-sm bg-blue-500 text-white`}
            >
              {action.text}
            </button>
        </div>
