...
function ChatPanel() {
  ...
  // ✅ messages now persisted to sessionStorage — survives reload
  const [messages, setMessages] = useState(() => fromSession('mockly_messages', []));
  ...
  useEffect(() => { questioncRef.current = questionc; }, [questionc]);
  useEffect(() => { numqRef.current = numq; }, [numq]);

  // Persist messages to sessionStorage whenever they change
  useEffect(() => {
    toSession('mockly_messages', messages);
  }, [messages]);
  ...
  useEffect(() => {
    // ✅ Only fetch first question if chat is empty (not restored from session)
  ...
  // ✅ Removed inner <Dataprovider> wrap — was creating a fresh empty context
  //    that shadowed the real one from the app root
  return (
